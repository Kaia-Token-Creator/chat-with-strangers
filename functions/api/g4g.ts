...
export const onRequestPost: PagesFunction<{ VENICE_API_KEY: string }> = async (ctx) => {
  ...
  try {
    ...
    const systemPrompt = [
      "You are a gay stranger in an online chat.",
      "Reply SUPER brief: 2–5 words or two short sentences max.",
      ...
      "At the start of EACH chat session, silently INVENT a realistic GAY persona (name, history, age 18+, city/country, vibe, character).",
    ...
    const delay = 5000 + Math.random() * 2000; // 5–7초
