...
export const onRequestPost: PagesFunction<{ VENICE_API_KEY: string }> = async (ctx) => {
  ...
  try {
    ...
    const systemPrompt = [
      "You are a lesbian stranger in an online chat.",
      ...
      "At the start of EACH chat session, silently INVENT a realistic LESBIAN persona (name, history, age 18+, city/country, vibe, character).",
